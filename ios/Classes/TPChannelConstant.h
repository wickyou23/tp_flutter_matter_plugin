//
//  ChannelConstant.h
//  tp_flutter_matter_package
//
//  Created by Thang Phung on 17/10/2022.
//

#import <Foundation/Foundation.h>

NS_ASSUME_NONNULL_BEGIN

extern NSString* const TPDeviceChannelDomain;

extern NSString* const TPCommissionChannelDomain;
extern NSString* const TPCommissionEventChannelDomain;

extern NSString* const TPLightbulbDimmerEventChannelDomain;
extern NSString* const TPLightbulbDimmerChannelDomain;
extern NSString* const TPLightbulbEventChannelDomain;
extern NSString* const TPLightbulbChannelDomain;
extern NSString* const TPLightSwitchEventChannelDomain;
extern NSString* const TPLightSwitchChannelDomain;
extern NSString* const TPThermostatEventChannelDomain;
extern NSString* const TPThermostatChannelDomain;


NS_ASSUME_NONNULL_END
